...
const FPOtp = () => {
    ...
    const userId = route.params?.dataOtp
    ...
    const getForgetOTP = async () => {
        ...
        try {
            ...
            const res = await AuthService.setForgetPasswordOtp(data)

            if (res?.status === true) {
                NavigationService.navigate('ForgetPassword', { getData: res?.data })
            } else {
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={{ ...styles.noaccount_txt, color: colors.tintText }}>Didn’t receive code?
                <Text style={{ color: colors.buttonColor }}> Resend again</Text></Text>

            <TouchableOpacity
                onPress={() => getForgetOTP()}
                style={{ ...styles.button_sty, backgroundColor: colors.buttonColor }}>
                {buttonLoader ? (
                    <ActivityIndicator size="small" color={'#fff'} />
                ) : (
                    <Text style={{ ...styles.signin_txt, color: colors.secondaryThemeColor }}> Verify</Text>
                )}
            </TouchableOpacity>
        </View>
    );
